...
def download_from_url(path, url):
  ...
  if found_file is None:
    ...
    print()
    tf.compat.v1.gfile.Rename(inprogress_filepath, filename)
    return filename
  else:
...
def encode_and_save_files(
  ...
  for tmp_name, final_name in zip(tmp_filepaths, filepaths):
    tf.compat.v1.gfile.Rename(tmp_name, final_name)
...
def shuffle_records(fname):
  ...
  tf.compat.v1.gfile.Rename(fname, tmp_fname)

  reader = tf.compat.v2.io.tf_record_iterator(tmp_fname)
  records = []
  ...
  tf.compat.v1.gfile.Remove(tmp_fname)
...
def all_exist(filepaths):
  ...
  for fname in filepaths:
    if not tf.compat.v1.gfile.Exists(fname):
      return False
  return True
